...
namespace pbbs {
  ...
  E* new_array_no_init(size_t n, bool touch_pages=false) { //true) {
    ...
    if (r == NULL) { std::cout << "Trying to allocate n = " << n << std::endl; assert(false); fprintf(stderr, "Cannot allocate space: %lu bytes", bytes); exit(1);}
...
//  size_t* failed_cas_attempts;

  template <typename E, typename EV>
  inline E fetch_and_add(E *a, EV b) {
 //   size_t cas_attempts = 0;
    volatile E newV, oldV;
    do {oldV = *a; newV = oldV + b;} // cas_attempts++;}
    while (!atomic_compare_and_swap(a, oldV, newV));
//    failed_cas_attempts[worker_id() * 128] += cas_attempts-1;
    return oldV;
  }
